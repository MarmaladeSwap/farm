...
contract BnbStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accMrmPerShare; // Accumulated BANANAs per share, times 1e12. See below.
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 MrmReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
